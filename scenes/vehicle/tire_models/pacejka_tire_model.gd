...
func update_tire_forces(slip: Vector2, normal_load: float, surface_mu: float) -> Vector3:
	...
	force_vec.x = pacejka(slip.x, pacejka_b, pacejka_c_lat, pacejka_d, pacejka_e, normal_load)
	force_vec.z = pacejka(slip.y, pacejka_b, pacejka_c_long, pacejka_d, pacejka_e, normal_load)
	force_vec.y = 0.0
